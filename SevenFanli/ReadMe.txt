========================================================================
    活动模板库 : SevenFanli 项目概述
========================================================================

应用程序向导已为您创建了此 SevenFanli 项目，作为编写动态链接库 (DLL) 
的起点。
...
SevenFanli.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(SevenFanli.h)
        GUID 定义			(SevenFanli_i.c)
        类型库				(SevenFanli.tlb)
        封送处理代码			(SevenFanli_p.c 和 
                                           dlldata.c）

SevenFanli.h
    此文件包含 SevenFanli.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 SevenFanli.pch 的预编译头 (PCH) 文件和
...
SevenFanlips.vcxproj
    此文件是用于生成代理/存根 (stub) DLL 的项目文件（若有必要）。主项目中的 IDL 
    文件必须至少包含一个接口，并且在生成代理/存根 (stub) DLL 之前必须先编译 IDL 
    文件。此进程生成 dlldata.c、SevenFanli_i.c 和 
    SevenFanli_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

SevenFanlips.vcxproj.filters
